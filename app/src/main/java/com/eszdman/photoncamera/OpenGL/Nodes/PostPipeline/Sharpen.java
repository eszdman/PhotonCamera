...
public class Sharpen extends Node {
    ...
    public void Run() {
        ...
        float sharpnessLevel = (float)Math.sqrt((CameraController.GET().getCaptureResult().get(CaptureResult.SENSOR_SENSITIVITY))* IsoExpoSelector.getMPY() - 50.)/14.2f;
        sharpnessLevel = Math.max(0.5f,sharpnessLevel);
        sharpnessLevel = Math.min(1.5f, sharpnessLevel);
        Log.d("PostNode:"+Name, "sharpnessLevel:" + sharpnessLevel + " iso:" + CameraController.GET().getCaptureResult().get(CaptureResult.SENSOR_SENSITIVITY));
