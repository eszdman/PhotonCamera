...
public final class SdrHeicEncoder {
     ...
     * the full size, and its single-frame video fallback rounds the encoded
     * width up to {@link #ENCODING_BLOCK_SIZE} while the container declares
     * the true width (a bitstream/extent mismatch), so single-frame is only
     * attempted for already block-aligned widths; other widths keep the grid
     * behavior, which declares the exact dimensions and crops padded edge
     * tiles. Any grid-off failure retries with the grid default, the proven
     * pre-existing path.
     *
