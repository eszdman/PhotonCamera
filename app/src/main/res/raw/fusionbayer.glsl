...
float laplace(sampler2D tex, vec4 mid, ivec2 xyCenter) {
    vec4 left = texelFetch(tex, xyCenter - ivec2(1, 0), 0).rgba,
    right = texelFetch(tex, xyCenter + ivec2(1, 0), 0).rgba,
    top = texelFetch(tex, xyCenter - ivec2(0, 1), 0).rgba,
    bottom = texelFetch(tex, xyCenter + ivec2(0, 1), 0).rgba;
