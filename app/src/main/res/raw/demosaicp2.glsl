...
float interpolateColor(ivec2 coords){
    ...
    //for(int i = 0; i<4; i++)if(green[i] > greenmax) usegreen = false;
    if(usegreen){
        float coeff[4];
        ...
        float greencenter = float(texelFetch(GreenBuffer, (coords), 0).x);
        return (greencenter*(coeff[0]+coeff[1]+coeff[2]+coeff[3])/4.);
    } else {
