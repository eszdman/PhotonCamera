...
void main() {
    ...
    vec3 lowrespix = vec3(textureBicubicHardware(LowRes, xyInterp).rgb);
    ...
    Output = vec4((texelFetch(InputBuffer, xy, 0).rgb)*(1.0+((absbr)*str*0.0)),1.0);
    else Output = vec4((texelFetch(InputBuffer, xy, 0).rgb)*(1.0+((absbr)*str*10.)),1.0);
    //Output*=0.9;
