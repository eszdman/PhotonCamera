...
void main() {
    ...
    if(fact1+fact2 != 1){
        ...
        //green[0] += imageLoad(gradTexture, ivec2(xy+ivec2(0, -1))).y;
        green[1] = imageLoad(inTexture, ivec2(xy+ivec2(-1, 0))).r;
        //green[1] += imageLoad(gradTexture, ivec2(xy+ivec2(-1, 0))).x;
        green[2] = imageLoad(inTexture, ivec2(xy+ivec2(1, 0))).r;
        //green[2] += imageLoad(gradTexture, ivec2(xy+ivec2(1, 0))).x;
        green[3] = imageLoad(inTexture, ivec2(xy+ivec2(0, 1))).r;
        //green[3] += imageLoad(gradTexture, ivec2(xy+ivec2(0, 1))).y;
        float grad[4];
        ...
        for (int j =-2;j<=2;j++)
        {
            ...
            for (int i =-2;i<=2;i++){
                ...
                vec2 div = abs(imageLoad(gradTexture, ivec2(xy+ivec2(i, j))).rg);
                HV += div*k;
                weights+=k;
            }
        }
        ...
        */
        ...
        /*
        if (HV.y > HV.x){
            Output.r = (green[1]*grad[1] + green[2]*grad[2])/(grad[1]+grad[2]);
        } else {
            Output.r = (green[0]*grad[0] + green[3]*grad[3])/(grad[0]+grad[3]);
        }
        */
        grad[0]*=HV.x;
        grad[1]*=HV.y;
        grad[2]*=HV.y;
        grad[3]*=HV.x;
        Output.r = (green[0]*grad[0] + green[1]*grad[1] + green[2]*grad[2] + green[3]*grad[3])/(grad[0]+grad[1]+grad[2]+grad[3]);
        imageStore(outTexture, xy, Output);
    } else {
        imageStore(outTexture, xy, imageLoad(inTexture,xy));
    }
}